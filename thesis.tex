...
TODO (important): ze muzeme rankovat ruzne atributy (napr. korektnost, relevanci, zabavnost a obtiznost), ale ze je to tezke (a nevim, jestli to vubec nekdo delal).
...
TODO (cela kapitola): doplnit relevantni citace (a poznamky), obrazky a grafy, dale promyslet (a otestovat na simulovanych datech) vyuziti modelu ve Smartoo (napr. PFA)
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{Relationship between challenge and skill \cite{flow}.}
  \label{fig:flow}
\end{figure}
...
but carefully select just a few questions, which are the most relevant to the learner and are not too easy or too difficult.
The generated questions are (usually) brand-new, so we necessarily need to rank them for relevance and difficulty estimation (and possibly other attributes as well) as described in \autoref{sec:questions-ranking-filtering},
...
It usually includes information about which skills have been mastered by the student
(and to which extent),
but other information may be also useful, e.g. enjoyment and motivation of the student \cite{learner-skill-models-review}.
...
(MaybeTODO: o tom, ze se zamerime hlavne na odhad prior knowledge (vs. current knowledge) (protoze uceni nas nezajima))
(MaybeTODO: predpoklad homogenity faktu i uzivatelu)
The most common student models for knowledge estimation are described in the following paragraphs.
\begin{myItemize}
\item \textbf{Logistic model}\\
  ...
  This model uses logistic function with a single paremeter $d$, which denotes the difficulty of the fact.
...
\end{equation}
TODO: parameters estimation, asi joint maximum likelihood estimation, napr: R. De Ayala. The theory and practice of item response theory. The Guilford Press, 2008.\\
MaybeTODO: ze neuvazuje uceni
...
The possible problem is hidden in constant $K$ -- if it's too large, estimation is unstable; if it's too small, estimation converges too slowly.
For this reason, instead of some consant, an attenuation function
\[
  K(m) = \frac{a}{1 + bm}
\]
($m$ is the order of the match, $a$ and $b$ are fixed parameters) can be used.
The main advantage of using the Elo model to estimate the prior knowledge and difficulties in compasion with the joint maximum likelihoood estimation technique (odkaz vyse) is its simplicity and intrinsic online nature.
Experiments suggest that it leads to similar results as the joint maximum likelikhood estimation \cite{slepe-mapy-time-elo}.

\item \textbf{Performance factors analysis}\\
  Performance factors analysis \cite{performance-factors-analysis} is similar to the logistic model presented above, but enables to capture learning.
  ...
  To take the order of answers into the account, performance factors analysis can be combined with the Elo model \cite{slepe-mapy} -- after each answer, estimate of knowledge parameter $k$ is updated according to the rule \ref{eq:elo}.


\item \textbf{Bayesian knowledge tracing}\\
...
Predpoklad je diskretni prechod z neumim do umim (coz je obvykle dost zjednodusujici, zapamatovani deklarativnich faktu je postupne), ale pouziva se u proceduralni znalosti
...
Although the ideal success rate may vary for different people (and even for person in different conditions)
...
However, several criteria should be considered to select the question which is the most useful for the student and difficulty is just one of them.
\begin{myItemize}
  \item \textbf{Relevance}\\
    ...
    Generally, the most relevant for the student is what he needs to pratice most, i.e.
  ...
  \item \textbf{Variability}\\
    ...
    variability of question types may also help (diversity increases enjoyment) \cite{question-gen-adapt-bayes}.
\end{myItemize}
