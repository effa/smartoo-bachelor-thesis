...
In this chapter I describe state-of-the-art int the field of question generation .
...
Every system use different approach and although usually the authors perform some kind of evaluation (e.g. by two people ranking generated questions as good or bad), it is not possible to say which approach is the best as the systems work in different settings, use different source texts from which the questions are generated, create different question types, have different target group and use different evaluation metrics.
...
and sometimes authors choose only one specific field or topic, such as linguistic \cite{question-gen-mitkov}
or biology \cite{question-gen-textbooks} [TODO: doplnit zamereni ostatnich clanku].
[TODO: o snaze v poslednich letech tento problem resit - shared task....]

Juding by reported performances of the systems, question generation is still very difficult task in the natural language processing and there is plenty of room for improvements or even completely new ideas.
Some systems proved to be usable in the offline mode, in which a teacher submits a text to the system, it outputs a list of questions and it is assumed that the teacher will filter them and make some corrections in the generated questions. Hopefully there is only a few questions that need to be discarded or corrected and so this semi-automated process of creating questions is faster than creating them fully manually
...
There are many approaches to the question generation,
but some common tasks can be identifed throughout the systems.
...
\item In the previous steps, questions are usually overgenerated, so it is important to filter them at the end. Instead of filtering, we can rank the questions and use some number of questions with the highest rank
(\autoref{sec:questions-ranking-filtering}).
\end{myEnumerate}

[TODO: projit text a clanky o gen. otazek a doplnit citace (a prip. podrobnosti, plus kontrola)]
...
\begin{myItemize}
  \item \textbf{Parsing}\\
    (je otazka, jestli to nemit az v nasledujici sekci.. kde to uz je)
...
Some of the features used in his classifier were:
  ...
  \item \textbf{Text trasnformations}\\
    ...
    TODO: \cite{question-improv-foster}
...
TODO: poznamka o \textit{content selection}, ze to nemusi byt vzdy skoro vsechno, ale treba jen 1 nebo 6 otazek na odstavec (1 Wolfe, 6 taky nekde bylo) nebo muzeme rovnou hledat pomoci klasifikatoru pouze nejrelevatnejsi vety (Agarwal \cite{question-gen-textbooks}). Ale vetsinovy pristup je otazky nadgenerovat a filterovat pozdeji.
...
Most question generation tools try to create questions directly from the text without the knowledge extraction step.
Usually, the system considers every sentence in the text one by one
...
which satisfies a given pattern, such as "The sentence starts with a term followed by a verb and an object."
...
The latter case is more difficult to implement, but it allows for simple modification of transformation rules and addition of new ones.

Patterns may be defined on syntactic or semantic level.
Many approaches are somewhere in between these two extremes and use some basic information about the meaning of  sentences (such as which words denotes terms), but do not rely on complete semantic roles labelling. Syntactic information used in patterns include: [maybeTODO: citace]
  \begin{myItemize}
    \item words in the sentence, such as "chairs", "are", "brown",
      or their canonical forms (\textit{lemmas}), e.g. "chair", "be", "brown".
    [TODO: ke vsem polozkam jeden konzistentni priklad, overit pomoci NLTK]
  \item \textit{part-of-speech tags} -- lexical categories (also known as word classes) of the word, such as noun, verb, adjective or determiner,
  \item \textit{syntax tree} of the sentence [TODO: rozvest, priklady frazi].
  \end{myItemize}
Semantic information which can be used to define patterns: [maybeTODO: citace]
  \begin{myItemize}
    \item named entities, and their types (pomoci named entity recognition -- odkaz),
    \item semanticke vztahy mezi slovy (hypernym, coordinate etc.) -- pomoci Wordnetu (co to je a citace, jiz nekde v kapitole Knowledge),
    \item semantic role labels (agent, patient, ...)  ... priklady z clanku... a citace.
  \end{myItemize}

[MaybeTODO: obrazek syntaktickeho stromu; demo shallow parsingu -- idealne srovnani techto dvou]

[MaybeTODO: citace konkretnich pripadu]


\subsection*{Examples of transformation rules}
TODO: projit clanky a vytahnout ukazky ruznych (syntaktickych) patternu

TODO: projit clanky a vytahnout ukazky ruznych semantickych) patternu


\noindent ---\\
TODO vzory z Wolfe, 70. leta, \textit{AutoQuest} \cite{questions-wolfe};
\\---

\noindent ---\\
Priklad [TODO: usporadat a vhodne okomentovat, a taky ocitovat]:
...
Answer: subject
\end{code}
hypernym found using WordNet). (TODO: uvest priklad)
\cite{question-gen-mitkov}
\\---

\noindent ---\\
TODO: vzory pomoci syntaktickych stromu, z \cite{question-gen-ceist} (\textit{Ceist system})
ale vymyslet vlastni priklad, ten jejich neobsahuje ukazku vyuziti stromu
\\---

\noindent ---\\
TODO: vzory souveti (spojky): delal jsem taky neco podobneho loni (napr. X because Y), ze par prikladu, neco bylo ve Wolfe (autoquest), dale semanticky v \cite{question-gen-connectives}
(connectives: beacuse, since, when, although, as a result, for example, for instance)
...
Answer: Because B.
\end{code}
Function \texttt{questionFom} searches for an auxiliary verb in A and if it finds one, it moves the auxiliary verb to the beginning of the sentence. If there is no auxiliary verb in the sentence, it adds appropriate auxiliary verb to the beginning of the sentence based on the tense of the main verb and the main verb is substituted for its infinitive (without to).  TODO: priklad

Problem: spatne se hledaji distraktory.
As an example, sentence "..." is tranformed into "...".



\noindent ---\\
TODO: semanticke: \cite{question-gen-mannem}, \cite{questions-eval}

  napr. \texttt{What |verb| |patient| |location|?} with answer being \texttt{agent} atp., taky. specifikuji semanticou roli, kterou zastava odpoved atp.
%\begin{code}
%Pattern:
%<*\arrowlinesplit*>
%Question:
...
Priklad patternu, ktery neni z cele vety, ale jenom z fraze \cite{mentor}:
\begin{code}
...
Answer: subject
\end{code}
Example: from phrase "Hamlet, written by Shakespeare" will be created question "Who wrote hamlet?" with the answer "Shakespear"


\subsection*{Correct answer selection}
There might be more possible transformations from a sentence to a question-answer pair.
In this case, only one question is usually generated.
Another strategy is to select a word from the sentence which would be the correct answer as a first sept and then generate a question from the rest of the sentence as a second step.
...
\textit{The-Mentor} system \cite{mentor}, created by Technical University of Lisbon,
...
They have discovered more then 1000 unique patterns, which is many more then tools with manually created patterns use. This allows form more specific patterns, such as the following:
\begin{code}
...
Answer: X
\end{code}
Example: sentence "Pablo Picasso was born in Spain." will be transformed to the quesiton "In which country was Pablo Picasso born?
...
In this section, we will consider situation when we have built knowledge graph
and want to create exercises from it, instead of from the original article.
...
There are some predicates which are not suitable for creating questions from them,
...
From the left graph, OntoQue could generate fill-in-the-blank question "Alan Turing was born in \sentenceGap .", while the right graph would be transformed into the question "Alan Turing birth date \sentenceGap ."
Another problem would be with answer... (TODO: je moc presna)
...
For this reason we might want to use more specific patterns to influence transformation from facts to sentences and simplify answers. For convenient retrieval of information from a knowledge graph, we can use SPARQL which was described in \autoref{sec:sparql}. As a simple example, rule for "When was X born?" question would look like this:
\begin{code}
...
But the great thing about knowledge graph is that we can select more facts and then ask a question which span more than one sentence. Example:
\begin{code}
PREFIX ont: <http://dbpedia.org/ontology/>
...
%\end{code}\\
%\noindent $\downarrow$ maps to
%\begin{code}



[TODO: ukazka sparql dotazu a nejakeho strukturovaneho vystupu (vcetne toho, na co chceme zeptat (spravna odpoved) z Procvicniku-v1, napr. Mohli se potkat?
...
\cite{mentor, question-gen-mannem, question-overgenerating-rating}.
The goal of the filtering is to discard questions which are:

\begin{myItemize}
  \item \textbf{invalid}\\
    Questions which does not even make sense can be created due to anaphoric references (TODO: autoref do sekce pre-processing?) Example of an invalid question with anaphoric reference is "Where is there?"
    (MaybeTODO: matching question-answer category (in The-Mentor system \cite{mentor}):
  ...
  \item \textbf{irrelevant}\\
    Even if a question is valid, it can be totally irrelevant for the student.
    As an example consider question TODO (for topic ...)

  \item \textbf{too easy}\\
    Some multiple choice questions may be too easy due to poor distractors.
    (Odkazat se na otazku se spatnymi distraktory uvedenou vyse.)

\end{myItemize}

As an alternative to filtering, we can rank questions first and then select $N$ best questions
(e.g. Mannem \cite{question-gen-mannem} selects $6$ best questions per a paragraph)
[TODO: nekde vybiralo 15 procent nejlepsich otazek]
We can even separately rank various parameters of questions, such as probability of correctness (validity), relevance and difficulty. I have chosen this approach in Smartoo
(\autoref{sec:smartoo-exercises-grading}), because it allows to separate the task of estimating parameters and the task of deciding which question is best for the student right now,
i.e. it allows to use adaptive practice.

Both filtering and ranking can be implemented either via heurstics decisions
or via a machine-learning based classifier.

Priklad heuristic ranking \cite{question-gen-mannem}: primary criterion is depth of the predicate from which is the question generated (to penalize questions form subordinate clauses) and a number of pronouns as a secondary criterion.

Heuristicke je ZREJME take DynaLearn \cite{question-gen-adapt-bayes}: selects question from a list of generated question based on its type (aims at variaty of asked question types) and difficulty.


Priklad ranking klasifikatoru: Heilman \cite{question-gen-heilman}: statisticky model pro question ranking (vyuziti strojoveho uceni)
features: question length, which wh-word (who, where, when, ...) was used (if any), if it includes negation (not, never, no), number of proper nouns, pronouns, adjectives etc., likelihoods of a question would appear in the text based on unigram and trigram language model and some more feautures..


TODO: \cite{question-overgenerating-rating}
