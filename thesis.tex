...
TODO: Obecny uvod, o co nam ted jde, exercises vs. questions, priklady a ze my se budeme ted zabyvat jen question generation, protoze je to bezne.
...
Konkretne:
- ucebnice biologie \cite{question-gen-textbooks}
- Mitkov, elementarni lingvistika \cite{question-gen-mitkov}
- Wolfe... \cite{questions-wolfe}
- Heilman, fakta obecne \cite{question-gen-heilman}
- zamereni na spojky (tj. ze souveti) \cite{question-gen-connectives}
- casto uceni se ciziho jazyka, to by taky chtelo nejakou citaci (i kdyz se tim nebudu zabyvat, tak aspon odkaz)
...
First step in question generation is to decide what we are going to ask about, i.e. what content (knowledge) we want to test (or practice).
Ideally we would like to select the most important and relevant content, but this is not easy.
Some common approaches to the content selection are listed and described below.

\begin{myItemize}
\item Every sentence from which the system is able to generate a question can be generated is used.
It usually means that the system considers every sentence in the text one by one
and tries to transform it to a question.
The transformation process is described in \autoref{sec:question-formation}.
For its simplicity this approach is used very often \cite{question-gen-mitkov, question-gen-connectives, questions-wolfe} [TODO: Wolfe - overit, doplnit dalsi citace].
It can lead to questions which are not much relevant,
but this issue can be solved by questions ranking and post-filtering (\autoref{sec:questions-ranking-filtering}).

\item Using a machine learning-based classifier to select only the sentences which are informative, relevant to the topic and have enough context in them to a question being generated from them.
This approach was tested by Agarwal \parencite{question-gen-textbooks}.
Some of the features used in his classifier were:
  ...
  \end{myItemize}
Agarwal also used another classifier to select a word from the sentence which would be the correct answer of the question generated from the sentence. For this classifier he used the following three features.
  \begin{myItemize}
...
\item MaybeTDO: vyber skupiny vet a z nich vytvorit otazku (aspon myslim, ze to nekde bylo.. jak bylo 6 otazek z jednoho odstavce???

\item Another approach is not to select appropriate content directly from the text, but rather from a knowledge graph built from the article by the process of knowledge extraction (\autoref{sec:relations-extraction}).
