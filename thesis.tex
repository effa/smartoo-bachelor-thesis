...
Although the ideal success rate may vary for different people,
it is for simplicity usually treated as a constant;
e.g. some systems target at 75 \% \cite{slepe-mapy,adaptive-practice-irt-math}.


\begin{figure}[h]
  \centering
  \begin{tikzpicture}[xscale=5, yscale=5]
  \draw [gray, fill=gray] (0,0) -- (0.1,0) -- (1,0.8) -- (0.8,1) -- (0,0.1) -- (0,0);
  \draw [thick, <->] (0,1) node [left] {$challenge$} -- (0,0) -- (1,0) node [below right] {$skill$};
  \large
  \node at (0.27,0.82) {frustration};
  \node at (0.7,0.7) {\textbf{flow}};
...
TODO: vsude doplnit citace (inspirace treba v clanku o Slepych mapach, nebo zdroje ALG)
...
(MaybeTODO: o tom, ze se zamerime hlavne na odhad prior knowledge (protoze uceni nas nezajima))
...
\end{equation}
TODO: graf, mozna oba\\
TODO: parameters estimation\\
MaybeTODO: neuvazuje uceni, existuji rozsireni, viz dale
...
\item \textbf{Elo model}\\
  ...
  (computed by the one-parameter logistic function \ref{eq:logistic}, or \ref{eq:logistic-mcq})
  the knowledge $k$ and diffiulcty $d$ are updated as follows:
  \begin{equation}\label{eq:elo}
  \begin{array}{l}
      k \gets k + K \cdot (R - P(k, d))\\
      d \gets d - K \cdot (R - P(k, d))
  \end{array}
\end{equation}
The difference $(R - P(k, d))$ express how much the result was unexpected (or ``surprise'').
The $K$ parameter influences the weight of the last match to the current estimate.
(a ze to muze byt fce)





O prior knowledge estimation:
If $K$ is too large, estimation is unstable; if it's too small, estimation is belated.
For this reason, it is of




podobne logistickemu modelu, ale nabize 2 mozne vyuziti: bud modelovani uceni nebo jako online metoda odhadu global prior knowledge \cite{slepe-mapy}
...
% ---------------------------  SECTION  ---------------------------
\section{Dynamic Target Difficulty Adjustment}
