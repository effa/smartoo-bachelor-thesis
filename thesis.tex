...
For example some systems try to test reading comprehension [TODO: citovat],
...
% ---------------------------  SECTION  ---------------------------
\section{Exercises from a knowledge graph}
\label{sec:exercises-from-knowledge-graph}

In this section we will consider situation when we have buitl knowledge graph
and want to create exercises from it, instead of from the original article.
Sometimes a single fact can be used to generate a meaningful question,
e.g. fact \texttt{(Alan Turing, birthdate, 23 June 1912)} leads to the question
\textit{When was Alan Turing born?} with the correct answer \textit{1912}.
But the great thing about knowledge graph is that we can select more facts and then ask questions which span more than one sentence.


To convenient retrieval of information from a knowledge graph, we can use SPARQL which was described in \autoref{sec:sparql}. As a simple example, rule for "When was X born?" question would look like this:
\begin{code}
PREFIX owl: <http://dbpedia.org/ontology/>
SELECT ?subject ?birthDate
WHERE {
  ?subject owl:birthDate ?birthDate .
}
<*\arrowlinesplit*>
Question: <*\emph{When was}*> subject <*\emph{born?}*>
Answer: birthDate
\end{code}

\begin{code}
PREFIX owl: <http://dbpedia.org/ontology/>
SELECT ?subject1 ?birth1 ?death1 ?subject2 ?birth1 ?death2
WHERE {
  ?subject1
      owl:birthDate ?birth1 ;
      owl:deathDate ?death1 .
  ?subject2
      owl:birthDate ?bith2 ;
      owl:deathDate ?death2 .
}
<*\arrowlinesplit*>
