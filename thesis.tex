...
obecne o tom kde to bylo nasazeno, ze to bylo testovano a tak

Smartoo is designed to collect both implicit and explicit feedback from users.
I analyze feedback from testing period..
...
Virtualenv + Python + DB (tested with SQLlite and MySQL, should also work for PosgreSQL)

dulezite knihovny:
Django==1.7.3 (web framework)
Jinja2==2.7.3 (server side templatin)
SPARQLWrapper==1.6.4 (interface to DBpedia public SPARQL endpoint)
nltk==3.0.1 (natural language processing)
rdflib==4.1.2 (working with RDF graphs)
wikipedia==1.4.0 (interface to Wikipedia)

JavaScript, HTML, CSS, Bootstrap, Angular, ...?
...
TODO zpetne vazby (slovni) - jenom strucny vytah (a napady do budoucna spis vyclenit do dalsi kapitoly)


% ---------------------------  SECTION  ---------------------------
\section{Performance of components}
\label{sec:analysis}
...
TODO: project structure description (proste adresarova struktura projektu se strucnym popisem)

simplified project structure:

\begin{tabular}{l l l}
\multicolumn{2}{l}{\texttt{+ common}} & bla bla\\
& \texttt{- fields.py} & bla bla\\
& \texttt{- utils/} & bla bla\\
& \texttt{- tests.py} & bla bla\\
\multicolumn{2}{l}{\texttt{+ abstract\_component}} & bla bla\\
& \texttt{- models.py} & bla bla\\
& \texttt{- component\_behavior.py} & bla bla\\

\end{tabular}

\begin{myItemize}
\item \texttt{common} -- bla bla
\begin{itemize}
\item \texttt{fields.py} -- bla bla
\item \texttt{utils} -- utility classes
\item \texttt{tests.py} -- bla bla
\end{itemize}

\item \texttt{abstract\_component}\\ bla bla
\begin{itemize}
\item \texttt{models.py}\\ bla bla
\item \texttt{component\_behavior.py}\\ bla bla
\end{itemize}
