...
\begin{myItemize}
  ...
  Instead of heuristics, we can use more sofisticated machine learning-based classifier to select only the sentences which are informative, relevant to the topic and have enought context to a question being generated from them. This approach was tested by Agarwal \parencite{question-gen-textbooks}.
Some of the features used in his classifier were:
  ...
  \item \textbf{Building knowledge graph}\\
...
TODO: poznamka o \textit{content selection}, ze to nemusi byt vzdy skoro vsechno, ale treba jen 1 nebo 6 otazek na odstavec (1 Wolfe, 6 taky nekde bylo) nebo muzeme rovnou hledat pomoci klasifikatoru pouze nejrelevatnejsi vety (Agarwal). Ale vetsinovy pristup je otazky nadgenerovat a filterovat pozdeji.
...
Most question generation tools try to create questions directly from text without the knowledge extraction step.
Usually the system considers every sentence in the text (without the filtered ones) one by one
and tries to transform it to a question
\cite{question-gen-mitkov, question-gen-connectives} [TODO: doplnit dalsi citace].
It can lead to questions which are not much relevant,
but this issue can be solved by questions ranking and post-filtering
(\autoref{sec:questions-ranking-filtering}).


............. TODO - odsud niz ...................

Na co se ptame -- na libovolny pojem, pripadne lze zkouset vybrat ten nejrelvantnejsi...
Agarwal \parencite{question-gen-textbooks} also used another classifier to select a word from the sentence which would be the correct answer of the question generated from the sentence. For this classifier he used the following three features.
  \begin{myItemize}
...
\item \textbf{Syntactic approach}\\
  Syntactic approach only relies on Patterns
  ...
  * hardcoded transformation rules (overit):
    ...
    * Muzemem mit oznacene pojmenovane entity, pak uz je to prechod k semantickemu (mitkov)
...
\item \textbf{Semantic approach}\\
  ...
  - Semanticky bez sablon: \cite{question-gen-mannem}
...
But the great thing about knowledge graph is that we can select more facts and then ask questions which span much more than one sentence.
[TODO: ukazka sparql dotazu (a odkaz do prislusne sekce dirve) a nejakeho strukturovaneho vystupu (vcetne toho, na co chceme zeptat (spravna odpoved) z Procvicniku-v1, mozna i 2 priklady, napr. Mohli se potkat? - ale taky moznost to nechat az na pristi sekci, tady se chceme bavit jen o vyberu contentu, ne formulaci otazek].

To ensure relevance of the questions, we can eithey employ a similar machine learning-based classifier as was described in the previous paragraph,
or we can overgenerate questions and then rank and filter them (\autoref{sec:questions-ranking-filtering}).
