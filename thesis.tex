...
\footnote{The selection is not actualy completely random, behaviors with better performance in the past are preferred over the behaviors with worse performance, see section \autoref{sec:smartoo-behaviors-selection} for details.} The described relationships are visualized on \autoref{fig:smartoo-architecture}.
...
The communication and the flow of data is depicted on \autoref{fig:data-flow-diagram}.
Individual processes are triggered by signals from client.
Time ordering of the signals and processes is showed on \autoref{fig:client-server-communication}.
...
You can see an example of a sentence tree on \autoref{fig:sentence-tree}.
...
method \texttt{build\_knowledge\_graph}, input: an article, output: knowledge graph
(odkaz do prislusnych sekci)
...
\begin{enumerate}
  ...
  \item postupne prochazime vsechny vety v clanku, filtrujeme pomoci jednoduche heuristiky ty bezkontextove (Maybe TODO: upresnit: konci teckou, rozumna delka(5-50), neobsahuje prounouns, parenthesis or quotations) a obsahujici alespon 1 pojem
...
Terms similarity heuristics is based on the number of commont types
$C(t_1, t_2) = |types(t_1) \cap types(t_2)|$:
$$
sim(t_1, t_2) = 2 \left( \frac{1}{1 + \exp({-|C(t_1, t_2)| / \kappa})} \right) - 1
$$

TODO: vysvetlit konstanty, 2 konkretni priklady podle poctu spolecnych typu

Maybe TODO: graf teto funkce v zavislosti na poctu spolecnych typu
