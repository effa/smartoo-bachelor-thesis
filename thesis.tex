...
\begin{myItemize}
  ...
  \item \textit{relation extraction} -- extraction of relations between the discovered named entities,
    for example \texttt{birhDate(Abraham Lincoln, 12 February 1809)}.
\end{myItemize}
...
obecne:
- hledame trojice (NE, relace, NE) -- viz RDF graph (subjekt a objekt jsou NE, predikat je relace
- uvest konkretni priklad trojice (napr. neco s Lincolnem)
- casto napred named entity recognition

review: \cite{rel-extract-review}

nlp-knizka \cite{nlp-python}:
- pomoci regexu: hledame sekvenci NE, nejaka slova oznacujici pozadovanou relaci NE,
priklad: ORG ... in ... LOC (vzory mohou obsahovat i POS-tagy, lemmata)
- vzory jsou zadany rucne, nebo muzeme zkusit se je naucit pomoci strojoveho uceni from a training corpus.

Priklad vzoru pro ...

\begin{myItemize}
  \item \emph{Abraham Lincol was born in 1809.}
  \item \emph{Abraham Lincol was born on 12 February, 1809.}
  \item \emph{Abraham Lincol was born in Hodgenville, Kentucky, on 12 February, 1809.}
\end{myItemize}
\begin{code}
Pattern: PERSON was born .*? (in | on) DATE
<*\arrowlinesplit*>
Relation: (PERSON, birthDate, DATE)
\end{code}
(Note: *? je lazy quantifier/operator... aby to bylo opravdu 1. datum ve vete (dulezite))

Takze moznosti (itemize)
- rucne psane vzory (regexy viz knizka): uvest priklad
- odvozeni vzoru pomoci strojoveho uceni


\parencite{triples-acquisition}
- pristup z knizky vyzaduje zadat relace, takze je to domain-dependent,
tento clanek se snazi o domain-independent relation extraction
subject-verb-object triples
concpet-verb-concept
- musi resit problem nedostatku kontextu ve vetach,
treba pronouns resolution, allocating missing subject or object or fragments (protoze pracuji s pozanmkami)
- priklad extrakce: (S (NP subtree = subject) (VP subtree - v nem se hleda nejzanorenejsi sloveso (verb), object se hleda v PP, NP a ADJP potomcich VP\_subtree (v NP a PP se vybere prvni podstatne jmeno or compound noun, v ADJP prvni pridavne jmeno.

Artequakt -- clanek ontology-based-knowledge-extraction.pdf: ?

pouziti strojoveho uceni (konkretne SVM) \cite{rel-extract-kernel}

Procvicnik-v1:
- date-event: pre-date date post-date => date = pre-date + post-date
- uplne analogicky pro person-event, place-cokoliv
