...
Many approaches are somewhere in between these two extremes and use some basic information about the meaning of  sentences (such as which words denotes terms), but do not rely on complete semantic roles labelling. Syntactic information used in patterns include:
  \begin{myItemize}
  ...
  \item \textit{syntax tree} of the sentence [TODO: rozvest, priklady frazi].
        MaybeTODO: odkaz na Tregex, Tsurgeon
  \end{myItemize}
Semantic information which can be used to define patterns: [maybeTODO: citace]
  \begin{myItemize}
    ...
    \item semanticke vztahy mezi slovy (hypernym, coordinate etc.) -- pomoci Wordnetu (co to je a citace, jiz nekde v kapitole Knowledge), prip. vztahy mezi pojmy v knowledge base such as DBpedia
    \item semantic role labels: agent, patient, cause, purpose, manner, locative or temporal argument etc.
  \end{myItemize}
...
As an typical example of a mostly syntactic approach (with a little bit help of the semantics)
\cite{question-gen-mitkov}:
\begin{code}
...
a vyuziva type of named entity
\begin{code}
...
Patterns based on connectives (beacause, so that, since, when, for example, ...) \cite{question-gen-connectives, questions-wolfe}:
\begin{code}
...
Example of pattern using semantic role labels \cite{question-gen-mannem, questions-eval}:
\begin{code}
...
MaybeTODO: spatne se hledaji distraktory pro clauses (napr. pri why - because questions)
...
Priklad heuristickeho filtru \cite{questions-wolfe}:
vyrad question-answer pair, pokud plati alespon 1 z nasledujicich: "otazka nebo odpoved ma vice nez 17 slov, otazka obsahuje slovo "and", prvni slovo odpovedi je relative pronoun or subordiante conjunction", otazka obsahuje carku", etc.
