...
The goal of \emph{computerized adaptive practice} [cite:treba adaptive-practice-irt-math]
is to make the practice as efficient as possible.
This is related to, but different from the \emph{computerized adaptive testing} [citovat]
...
Some of them focus on the procedural knowledge (such as solving math problems) [citace,konkretni systemy, napr. Math Garden a clanek o souvisejim vyzkumu adaptive-practice-irt-math],
...
Alhough the ideal success rate may vary for different people,
...
MaybeTODO: flow diagram (svuj, ale citace na myslenku)

Intelligent tutoring systems use attempts (answers) of learners to estimate both skill (knowledge) of each learner and the difficulty of all facts (or tasks).
As most people use these tutors repeatedly and not only once, the process of learning can be also captured
and the systems may even have special estimate parameter for each learner-fact pair \cite{slepe-mapy}.
...
% NOTE: kdyby toho bylo hodne, je to mozne rozdeleit do 2 kapitol (Student Modeling a Knowledge Estiamtion)
...
obecne o tom, co to je \textit{student model}, rika pravdepodobnost, ze student odpovi spravne na nejakou otazku
... taky o tom, ze se zamerime hlavne na odhad prior knowledge (protoze uceni nas nezajima)


\begin{myItemize}
\item \textbf{Logistic model}\\
  One parameter logistic model, also called Rash model; neuvazuje uceni
  znalost studenta daneho faktu dana parametrem $k$ a obtiznost otazky dana parametrem $d$, vypada odhad spravne odpovedi nasledovne:
\begin{equation}\label{eq:logistic}
P(k, d) = \frac{1}{1 + e^{-(k - d)}}
\end{equation}
Vlastnosti:, ze je to vzdy mezi 0 a 1
In case of multiple choice questions with $n$ options (tj. pravdepodobnost nahodneho uhadnuti je $\frac{1}{n}$):
\begin{equation}\label{eq:logistic-mcq}
P(k, d) = \frac{1}{n} + \left( 1 - \frac{1}{n} \right) \cdot \frac{1}{1 + e^{-(s - d)}}
\end{equation}
(Mozne TODO: graf)
...
\item \textbf{Performance factor analysis}\\
